            ...
            <div className="hidden md:flex flex-col rounded-2xl bg-gray-100 text-white p-3 border justify-center items-center sm:flex-row gap-4">
                {tabs.map((tab) => (
                    <button
                        ...
                        key={tab.label}
                        className={`rounded flex justify-center items-center font-semibold py-2 px-4 w-full cursor-pointer ${tab.id === 'transactions' ? 'bg-white text-blue-500' : 'text-gray-500 hover:bg-white hover:text-blue-500'
                            }`}
                    >
